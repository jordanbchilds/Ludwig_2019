...
#samtools depth -f ls_bam_files.txt -a -H -q 30 -Q 18 -r chrM -d 0 -o alignment_stats/depths_qfilt_30.txt
...
#samtools coverage -b ls_bam_files.txt --excl-flags UNMAP,SECONDARY,QCFAIL,DUP -q 30 -Q 18 -r chrM -o alignment_stats/mean_coverage_qfilt_30.txt;
...
#echo "SRRfile	#rname	startpos	endpos	numreads	covbases	coverage	meandepth	meanbaseq	meanmapq" > alignment_stats/all_coverages_qfilt_30.txt;
...
# echo "${i_nodir}	`grep chrM alignment_stats/coverage_qfilt_${i_nodir}.txt;`" >> alignment_stats/all_coverages_qfilt_30.txt
