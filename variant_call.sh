...
do
  ...
  if [ -f "bcftools_out/${rt}_calls.vcf" ]; then
    ...
    samtools view bamc/${rt}.bam chrM -h -u | bcftools mpileup - --no-BAQ --max-depth 999999 --fasta-ref ${ref} -q 18 -Q 20 --annotate FORMAT/AD,FORMAT/ADF,FORMAT/ADR,FORMAT/SP,FORMAT/RPB,INFO/AD,INFO/ADF,INFO/ADR,INFO/SP --threads 8 -Ov --output bcftools_out/${rt}_mpileup.vcf;
